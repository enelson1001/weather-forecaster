...
//!
//! Use Bounce Buffer since were are also using Wifi, otherwise wifi will cause disply to shift on screen.
//! See https://www.esp32.com/viewtopic.php?f=12&p=94343
...
impl Default for PanelConfig {
    fn default() -> Self {
        Self {
            ...
            bits_per_pixel: 0,
            num_fbs: 1,
            bounce_buffer_size_px: (10 * HOR_RES) as usize,
            sram_trans_align: 8,
...
impl Default for TimingsConfig {
    fn default() -> Self {
        Self {
            ...
            horz_res: HOR_RES, // 800
            vert_res: VER_RES, // 480
...
impl LcdPanel {
    pub fn new(
        ...
        unsafe {
            // create panel
            esp_idf_svc::sys::esp!(esp_lcd_new_rgb_panel(&panel_config, &mut panel))?;

            // reset panel
            esp_idf_svc::sys::esp!(esp_lcd_panel_reset(panel))?;

            // initialize panel
            esp_idf_svc::sys::esp!(esp_lcd_panel_init(panel))?;
        };
        ...
        T: IntoIterator<Item = lvgl::Color>,
    {
        ...
        unsafe {
            esp_idf_svc::sys::esp!(esp_lcd_panel_draw_bitmap(
                self.panel,
                sx,
                sy,
                ex,
...
impl Drop for LcdPanel {
    fn drop(&mut self) {
        esp_idf_svc::sys::esp!(unsafe { esp_lcd_panel_del(self.panel) }).unwrap();
    }
}
